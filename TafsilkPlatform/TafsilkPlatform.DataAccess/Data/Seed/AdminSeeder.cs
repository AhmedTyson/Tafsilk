...
namespace TafsilkPlatform.DataAccess.Data.Seed
{
    public static class AdminSeeder
    {
        ...
        public static void Seed(ApplicationDbContext db, IConfiguration config, ILogger logger)
        {
            ...
            var adminEmail = config["Admin:Email"] ?? "admin@tafsilk.com";
            var adminPassword = config["Admin:Password"] ?? "Admin@2024!Secure";

            if (string.IsNullOrWhiteSpace(adminEmail) || adminEmail == "admin@tafsilk.local")
            {
                logger.LogWarning("Admin email not configured. Using default 'admin@tafsilk.com'. Set 'Admin:Email' in appsettings.json.");
            }

            if (string.IsNullOrWhiteSpace(adminPassword) || adminPassword == "Admin@123!")
            {
                logger.LogWarning("Admin password not configured. Using default. Set 'Admin:Password' in appsettings.json and change after first login.");
            }
