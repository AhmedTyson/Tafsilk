   ...
   if (!canReview)
{
      ...
      TailorName = order.Tailor?.ShopName ?? "Unknown",
       ServiceType = order.OrderType,
          OrderDate = order.CreatedAt.DateTime,
           OrderPrice = (decimal)order.TotalPrice
    };
 ...
 TempData["Error"] = "حدث خطأ أثناء تحميل صفحة التقييم";
  ...
  [HttpPost]
    ...
    public async Task<IActionResult> Create(Guid orderId, CreateReviewRequest request)
  {
   try
    {
   if (!ModelState.IsValid)
{
 ...
 // Submit review via service
  var result = await _reviewService.SubmitReviewAsync(orderId, customerId, request);
    ...
    if (!result.IsSuccess)
 {
    ...
    return RedirectToAction(nameof(Create), new { orderId });
          ...
          TempData["Success"] = "شكراً لتقييمك! تم إرسال تقييمك بنجاح.";
            return RedirectToAction(nameof(ReviewSuccess), new { reviewId = result.Data });
 }
     catch (Exception ex)
     {
_logger.LogError(ex, "Error submitting review for order {OrderId}", orderId);
